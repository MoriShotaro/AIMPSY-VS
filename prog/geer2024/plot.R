...
g_Fig_1_1 <- df_yr_ele %>%
  filter(Sr%in%c('NEA'),Sc%in%c('NoPOL','w/o grid'),
  ...
  mutate(Sv=factor(Sv,levels=rev(eleyr$Variable)),
         Sc=factor(Sc,levels=c('NoPOL','w/o grid','w/ grid')),
...
g_Fig_4_1 <- df_yr_ele %>%
  filter(Sr%in%c('JPN'),Sc%in%c('NoPOL','w/o grid','w/ grid'),
         Sv!='Sec_Ene_Ele_Cur',Sv!='Sec_Ene_Ele_Sto_Los',Sv!='Sec_Ene_Ele_Trd_Los') %>%
  mutate(value=case_when(Sv%in%flag_eleyr~-value,
                         TRUE~value)) %>%
  mutate(Sv=factor(Sv,levels=rev(eleyr$Variable)),
...
g_Fig_6_0 <- df_da_ele %>% 
  filter(Sr=='JPN',Sc%in%c('w/o grid','w/ grid'),value!=0) %>%
...
g_Fig_6_1 <- df_da_ele %>% 
  filter(Sr=='JP-KT',Sc%in%c('w/o grid','w/ grid'),value!=0) %>%
  mutate(value=case_when(Sv%in%flag_elehr~-value,
                         TRUE~value),
         D=as.Date(D-1,origin=paste0(plot_year,'-01-01'))) %>% 
  mutate(Sv=factor(Sv,levels=rev(elehr$Variable)),
...
g_Fig_6_2 <- df_da_ele %>% 
  filter(Sr=='JP-KS',Sc%in%c('w/o grid','w/ grid'),value!=0) %>%
  mutate(value=case_when(Sv%in%flag_elehr~-value,
                         TRUE~value),
         D=as.Date(D-1,origin=paste0(plot_year,'-01-01'))) %>% 
  mutate(Sv=factor(Sv,levels=rev(elehr$Variable)),
...
g_Fig_6_3 <- df_da_ele %>% 
  filter(Sr=='JP-HD',Sc%in%c('w/o grid','w/ grid'),value!=0) %>%
  mutate(value=case_when(Sv%in%flag_elehr~-value,
                         TRUE~value),
         D=as.Date(D-1,origin=paste0(plot_year,'-01-01'))) %>% 
  mutate(Sv=factor(Sv,levels=rev(elehr$Variable)),
...
g_Fig_6_4 <- df_da_ele %>% 
  filter(Sr=='JP-TH',Sc%in%c('w/o grid','w/ grid'),value!=0) %>%
  mutate(value=case_when(Sv%in%flag_elehr~-value,
                         TRUE~value),
         D=as.Date(D-1,origin=paste0(plot_year,'-01-01'))) %>% 
  mutate(Sv=factor(Sv,levels=rev(elehr$Variable)),
...
g_Fig_6_5 <- df_da_ele %>% 
  filter(Sr=='JP-CB',Sc%in%c('w/o grid','w/ grid'),value!=0) %>%
  mutate(value=case_when(Sv%in%flag_elehr~-value,
                         TRUE~value),
         D=as.Date(D-1,origin=paste0(plot_year,'-01-01'))) %>% 
  mutate(Sv=factor(Sv,levels=rev(elehr$Variable)),
...
g_3_1 <- dmda_t %>% 
  ...
  filter(Sc%in%c('NoPOL','500C')) %>%
  group_by(I) %>% 
  complete(Sc=c('NoPOL','500C'),Y=seq(2005,2050,5),fill=list(value=0)) %>% 
  ggplot() +
